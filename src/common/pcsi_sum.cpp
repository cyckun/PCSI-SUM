...
void interpolate_poly_with_dummy(std::vector<uint64_t>::iterator poly_offset, std::vector<uint64_t>::const_iterator x_offset, std::vector<std::vector<uint64_t>>::const_iterator y_offset, std::size_t bin_num, PCSIContext& ctx) {
...
std::vector<uint64_t> server_opprf(const std::vector<uint64_t>& eles, PCSIContext& ctx) {
    ...
    std::uniform_int_distribution<uint64_t> dist(0, (1ull << ctx.max_bitlen) - 1);
    ...
    interpolate_poly(polys, X, masks, ctx);

    sock->Send((uint8_t *)polys.data(), ctx.mega_bins_num * ctx.poly_bytelength);
    sock->Close();
...
uint64_t exec(const std::vector<uint64_t>& inputs, PCSIContext& ctx, const std::vector<uint64_t>& data) {
    ...
    if (ctx.role == CLIENT) {
        ...
        std::vector<osuCrypto::block> ot_output = recv_osn(dest, bins, ctx);
